...
def create_draft(
    ...
    *,
    ...
    audit = _append_audit(
        audit,
        "create_draft",
        {"upload_id": upload_id, "model_type": model_type},
        actor=owner_orcid,
    )
...
def archive_draft(cfg: OrchestratorConfig, *, draft_id: str, actor_orcid: Optional[str] = None) -> dict[str, Any]:
    p = _paths(cfg, draft_id)
    if not p.draft_json.exists():
        raise FileNotFoundError(draft_id)
    ...
    audit = _read_json(p.audit_json) if p.audit_json.exists() else _init_audit(draft_id)
    audit = _append_audit(audit, "archive_draft", {}, actor=actor_orcid or draft_obj.get("owner_orcid"))
